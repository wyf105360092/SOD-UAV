...
class SPPCSPC_Biatt(nn.Module):
    # CSP https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=(5, 9, 13)):
        super(SPPCSPC_Biatt, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(c_, c_, 3, 1)
    ...
    def forward(self, x):
        ...
        y1 = self.cv6(self.cv5(torch.cat([x1] + [m(x1) for m in self.m], 1)))
        y2 = self.cv2(x)
        return self.attention(self.cv7(torch.cat((y1, y2), dim=1)))
...
class Yolov7_E_ELAN_NECK_EMA1(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_E_ELAN_NECK_EMA1, self).__init__()
    ...
    def forward(self, x):
        ...
        x6 = self.conv6(x5)
        x_concat = torch.cat([x1, x2, x3, x4, x5, x6], dim=1)
        x_final = self.attention(self.conv7(x_concat))
        return x_final


class Yolov7_ELAN_EMA1(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_ELAN_EMA1, self).__init__()
...
class Yolov7_ELAN_EMA2(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_ELAN_EMA2, self).__init__()
        ...
        self.conv6 = Conv(hidc, hidc, k=3, act=act)
        self.conv7 = Conv(hidc * 4, ouc, k=1, act=act)
        self.attention1 = EMA(hidc)

    def forward(self, x):
        x1, x2 = self.conv1(x), self.conv2(x)
        x3 = self.attention1(x2)
        x4 = self.attention1(x3)
        x5 = self.attention1(x4)
        x6 = self.attention1(x5)
        x_concat = torch.cat([x1, x2, x4, x6], dim=1)
        x_final = self.conv7(x_concat)
        return x_final

class Yolov7_ELAN_EMA3(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_ELAN_EMA3, self).__init__()
    ...
    def forward(self, x):
        ...
        x6 = self.attention1(x5)
        x_concat = torch.cat([x1, x2, x4, x6], dim=1)
        x_final = self.attention2(self.conv7(x_concat))
        return x_final
...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.ModuleList(BottleneckC3(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.Sequential(*(BottleneckC3(c_, c_, shortcut, g, k=((1, 1), (3, 3)), e=1.0) for _ in range(n)))
...
class Bottleneck_EMSC(BottleneckC3):
...
class C2f_EMSC6(C2f):
    def __init__(self, c1, c2, n=6, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Bottleneck_EMSC(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))


class Bottleneck_EMSCP(BottleneckC3):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        super().__init__(c1, c2, shortcut, g, k, e)
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, k[0], 1)
