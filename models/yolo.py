...
class IDetect_DyHead(nn.Module):
    stride = None  # strides computed during build
    export = False  # onnx export
    end2end = False
    include_nms = False
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        super(IDetect_DyHead, self).__init__()
        self.nc = nc  # number of classes
        self.no = nc + 5  # number of outputs per anchor
        self.nl = len(anchors)  # number of detection layers
        self.na = len(anchors[0]) // 2  # number of anchors
...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, TSCODE_Detect):
            ...
            # print('Strides: %s' % m.stride.tolist())
        if isinstance(m, (IDetect, IDetect_DyHead, IDetect_AFPN_P345, IDetect_AFPN_P2345)):
            s = 640  # 2x min stride
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s))])  # forward
            check_anchor_order(m)
            m.anchors /= m.stride.view(-1, 1, 1)
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if self.traced:
                if isinstance(m, Detect) or isinstance(m, (IDetect, IDetect_DyHead, IDetect_AFPN_P2345)) or isinstance(m, IAuxDetect) or isinstance(m, IKeypoint) or isinstance(m, TSCODE_Detect):
                    break

            if profile:
                c = isinstance(m, (Detect, IDetect, IDetect_DyHead, IDetect_AFPN_P345, IDetect_AFPN_P2345, IAuxDetect, IBin, TSCODE_Detect))
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            ...
            elif isinstance(m, (IDetect, IDetect_DyHead, IDetect_AFPN_P345, IDetect_AFPN_P2345, IAuxDetect)):
                m.fuse()
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
                 SPP, SPPF, SPPCSPC, SPPCSPC_Biatt, GhostSPPCSPC, MixConv2d, Focus, Stem, GhostStem, CrossConv,
                 ...
                 Yolov7_E_ELAN_NECK_EMA1, C2f_EMSC3, C2f_EMSC6, C2f_EMSCP3, C2f_EMSCP6,
                 Yolov7x_E_ELAN, Yolov7_ELAN_EMA1, Yolov7_ELAN_EMA2, Yolov7_ELAN_EMA3, CBAM]:
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            c2 = ch[f] // 2
        elif m in [Detect, TSCODE_Detect, IDetect, IDetect_DyHead, IDetect_AFPN_P345, IDetect_AFPN_P2345, IAuxDetect, IBin, IKeypoint]:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='/home/wangyifu/yolo/SOD-yolov7/cfg/improved/yolov7-bifpn2.yaml', help='model.yaml')
